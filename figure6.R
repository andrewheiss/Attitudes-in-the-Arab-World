#-----------------------------------------------
# Figure 6: Grid plot for coefficient t-values
#-----------------------------------------------
# Create dataframe of t-values for combined model and all individual models
values <- data.frame(rbind(summary(all.countries)$coefficients[, "t value"],
                           summary(original4)$coefficients[, "t value"],
                           t(sapply(seq(country.models), 
                                    FUN=function (x) summary(country.models[[x]])$coefficients[, "t value"]))
))
...
fig6 <- p + geom_tile() + scale_fill_gradient2(low="#D7191C", mid="white", high="#2C7BB6") + 
  #   geom_text(aes(label=stars, color=value), size=8) + scale_colour_gradient(low="grey30", high="white", guide="none") +
  geom_text(aes(label=stars), color="black", size=5) + 
  labs(y=NULL, x=NULL, fill="t-value") + geom_vline(xintercept=2.5, size=1.5, color="grey50") + 
